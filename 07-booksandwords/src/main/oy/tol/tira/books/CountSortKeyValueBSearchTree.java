...
public class CountSortKeyValueBSearchTree<V extends Comparable<V>> {
...
public Pair<String, V>[] toSortedArray() {
    ...
    for (Pair<String, V> pair : sorted) {
        if (pair != null && pair.getValue() != null) {
            int value = (int) pair.getValue();
            lowBound = Math.min(lowBound, value);
            upBound = Math.max(upBound, value);
        }
    }
    int[] count = new int[upBound - lowBound + 1];
    for (Pair<String, V> pair : sorted) {
        if (pair != null && pair.getValue() != null) {
            int value = (int) pair.getValue();
            count[value - lowBound]++;
        }
    }
    for (int i = count.length-1; i > 0; i--) {
        count[i-1] += count[i];
    }
    Pair<String, V>[] sorted2 = new Pair[sorted.length];
    for (int i = sorted.length - 1; i >= 0; i--) {
        if (sorted[i] != null && sorted[i].getValue() != null) {
            int value = (int) sorted[i].getValue();
            sorted2[count[value - lowBound] - 1] = sorted[i];
            count[value - lowBound]--;
        }
    }
