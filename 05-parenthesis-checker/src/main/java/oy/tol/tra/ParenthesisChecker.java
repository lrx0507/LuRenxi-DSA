...
public class ParenthesisChecker {
   ...
   public static int checkParentheses(StackInterface<Character> stack, String fromString) throws ParenthesesException {
      ...
      int cnt1 = 0;
      int cnt2 = 0;
      for (int i = 0; i < (int) fromString.length(); ++i) {
         if (fromString.charAt(i) == '(' || fromString.charAt(i) == '[' || fromString.charAt(i) == '{') {
            if (stack.size() == stack.capacity()) {
               throw new ParenthesesException("stack's capacity is not enough.", ParenthesesException.STACK_FAILURE);
            }
         ...
         if (fromString.charAt(i) == ')' || fromString.charAt(i) == ']' || fromString.charAt(i) == '}') {
            ...
            if (stack.size() == 0) {
               ...
               char tmp = stack.peek();
               if (tmp == '(' && fromString.charAt(i) == ')') {
                  stack.pop();
                  flag = true;
               } else if (tmp == '[' && fromString.charAt(i) == ']') {
                  stack.pop();
                  flag = true;
               } else if (tmp == '{' && fromString.charAt(i) == '}') {
                  stack.pop();
